...
\end{align}
%
where $J_k(t)$ is the computed cost over the simulated trajectory over the time horizon $h$ using the gear-ration mode $k$. If the minimize cost is the squared actuator torque then:
%
\begin{align}
J_k(t) = \int_{t}^{t+h}{  \vec{\tau}_k^T \vec{\tau}_k \, d\hat{t} }
\end{align}
%
where $\vec{\tau}_k$ is the computed torque in predictive simulation with the base policy when using the gear-ratio mode $k$. The variable $\hat{t}$ is use to represent time in the simulation.  An additional instantaneous cost $Q$ is added when the gear-ratio mode is changed. 


\begin{figure}[H]
	\centering
		\includegraphics[width=0.9\textwidth]{rollouttrajs.png}
