...
%
\begin{align}
\dot{J}_k(t) &\leq C_k^{max} -  0   +   \int_{t}^{t+h}{ \dot{C}_k^{max} \, d\hat{t} }  \leq C_k^{max}  +   \dot{C}_k^{max} h
\end{align}
%
...
\end{align}
%
This is useful because, for the closed-loop system, the error dynamic is stable and linear. 

%
\begin{align}
\dot{\hat{\vec{x}}}_e = A \hat{\vec{x}}_e  \quad \Rightarrow \quad  \hat{\vec{x}}_e(\hat{t}) = e^{A \hat{t}} \hat{\vec{x}}_e(\hat{t}=0)
\end{align}
%
%
\begin{align}
\vec{\hat{x}}_e( \hat{t} , t ) = e^{A \hat{t}} \hat{\vec{x}}_e(t) + \vec{x}_e(t) = \left[ e^{A \hat{t}} + 1 \right] \vec{x}_e(t)
\end{align}
%
...
%
\begin{align}
\frac{\partial \vec{\hat{x}}}{\partial t}  = \left[ e^{A \hat{t}} + 1 \right] \dot{\vec{x}}_e(t)
\end{align}
%

Hence trajectory sensitivity for two 
