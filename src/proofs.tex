...
It can be shown that there is a minimum delay $\Delta t$ between a back-and-forth gear-shift sequence. Hence, no arbitrary fast chattering between gear-ratio (zeno behavior [REF switched system]) is possible. 
...
%
\begin{align}
J_k^A &= \int_{t}^{t+\Delta t}{         C_k  d\hat{t}  }   \\
J_k^B &= \int_{t+\Delta t}^{t+h}{       C_k  d\hat{t}  }   \\
