...
\begin{figure}[htb]
	\centering
		\includegraphics[width=0.65\textwidth]{arm_config.png}
...
The initial control implementation for DSDM actuator prototypes used a \textit{NI compact rio} where feedback loops were running at very high-sampling rate on a FPGA and a real-time micro-controller \cite{girard_two-speed_2015}. This type of system was however poorly suited to scale to multi-DoF systems, and implementing complex planning algorithms. Hence, for the DSDM-Arm the architecture was design to: easily scale to multiple DoF, make possible the use of high-level programming languages and facilitate connections with motion planning algorithms. However this came at the cost of loosing some control on the low-level loop implementations. The performance of actuator-level DSDM controller has decreased with the new implementation. To improve the system, DSDM controller should be implemented directly on micro-controllers, handling all actuator-level functions, at a higher sampling rate. One implementation issue that could also be improved, is that velocity measurements, at low speeds in high-speed mode, are very noisy because of resolution problems. An encoder with improved resolution, or direct an angular velocity sensor should be used to improve the performance. 
