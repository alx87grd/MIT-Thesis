\chapter{Variable Gear-ratio Actuator with Fast and Seamless Transitions }
...
Regarding power-throughput, as discussed before and illustrated at Fig. \ref{fig:emcurve}, electromagnetic actuators are typically characterized by a flat force curve for most of their range of speed, leading to maximum power been only available at high velocity. Fluidic actuator are typically characterized by a force curves doping quadratically with velocity (related to pressure losses in valves orifices), as illustrated at Fig. \ref{fig:fluidiccurve}. All in all, EM and fluidic actuators cannot transmits their maximum power over their full range of speed, and variable transmission can make their power be available on a much wider range of speed.

\begin{figure}[htb]
        \centering
				\subfloat[Electromagnetic transducer]{
				\includegraphics[width=0.40\textwidth]{EM_curve.png}
				\label{fig:emcurve}
				}
        \subfloat[Fluidic transducer]{
				\includegraphics[width=0.40\textwidth]{fluid_curve.png}
...
\paragraph{Vehicle Powertrains} While the actuator work in robotics have been focused on impedance and bandwidth issues, in the powertrain field the torque-speed matching issue is predominant, since power density and efficiency are critical for mobile systems. The idea of using multiple gear ratios with electric motors has been explored occasionally, to improve efficiency and power density \cite{mckeegan_antonovs_2011}. A twin motor configuration has been proposed for smooth gear shifting, where each motor shifts at a different timing \cite{bologna_electric_2014}. Also, a dual motor configuration using a planetary coupling and non-back-drivable worm-gears was proposed for a mobile robot powertrain \cite{lee_new_2012}. Multiple gear-ratio powertrains provide effective solutions for torque-speed matching, but are not adapted to make gear shifts while interacting with dynamic environments. Fig. \ref{fig:forcefidelity} shows typical transmitted force profile during gear-shifts with powertrain shifting mechanisms. Simplest manual transmission using dog clutch do not transmit torque at all during a shift, while more advance systems such as dual-clutch transmission can supply torque during the transition, but with poor fidelity compared to what is typically expected from an actuator in a robotic context.
%
\begin{figure}[htp]
	\centering
...
\end{figure}
%
As illustrated at Fig. \ref{fig:typeloads}, force fidelity requirement for vehicle powertrain are not very important since the load is always a very large vehicle inertia which will act as a very strong low-pass filter. However, robotic system can be interacting with all kind of load without filtering characteristics. For instance, for a robot holding a large mass against gravity or compressing a spring, it would be catastrophic if the force drop during a gear-shift. The output needs to be always fully under control in those situations.
%
\begin{figure}[htp]
	\centering
	...
	\label{fig:typeloads}
\end{figure}
Hence, powertrain technologies cannot be applied directly to general purpose robotic system.
...
The presented actuator in this chapter, address the issue of improving available power and efficiency over a wide range of operating speeds, which has rarely been addressed in the robotics literature. Also, the proposed multiple gear-ratios actuator is an alternative approach to impedance variation that can achieve many order-of-magnitude changes. The main novel contribution is the methodology for gear shifting that is adapted to a robotics context. A mechanical architecture where two motors are coupled using a 3-ports gearbox and a brake is used in conjunction with a novel controller to provide full control of the output during gear shifting, which is a key enabling feature for robotic applications.
...
Regarding the dual-motor architecture used in the proposed VGA design. Similar architectures have been used in hybrid car powertrains and special actuators, but not in a the context of addressing gear shifting issues, i.e. the control of transitions between discrete operating modes. Here this architecture is used in conjunction with a novel controller to provide full control of the output during gear shifting. A preliminary version of this work have been published in \cite{girard_two-speed_2015}, but this chapter include a more throughout analysis and new algorithms for fast gear-shift even during impacts. 
...
If M1 jams, then the situation is the same as if the brake would be engaged, and the capability of high-force mode can still available.
...
If M2 is no longer able to transmit any torque, M1 can still be used to move the output. Assuming the reduction stage of M2 is irreversible or barely backdrivable (meaning its associated moving part is the differential is still fixed and can sustain reaction forces) then the capability of high-speed are fully available.
...
\subsection{Hybridness due to Discrete Operating Modes}
...
\begin{align}
\text{High-speed mode: } \quad \left[ I_o + R_1^2 I_1 \right] \dot{w}_o +  \left[ b_o \right] \dot{w}_o  &= \left[ R_1 \right] \tau 
\label{eq:dsdm_output_R1} \\
\text{High-force mode: } \quad \left[ I_o + R_2^2 I_2 \right] \dot{w}_o +  \left[ b_o \right] \dot{w}_o  &= \left[ R_2 \right] \tau 
...
\begin{align}
\left[ I_o + R_k^2 I_k \right] \dot{w}_o +  \left[ b_o \right] \dot{w}_o  &= \left[ R_k \right] \tau \quad k \in {1,2}
...
This section presents experimental results with the 
...
\begin{figure}[H]
	\centering
		\includegraphics[width=0.95\textwidth]{dsdm_manual_shift.pdf}
	\caption[DSDM actuator torque control]{DSDM actuator with torque and mode commands set from a joystick: down-shift }
...
\begin{figure}[H]
	\centering
		\includegraphics[width=0.95\textwidth]{dsdm_position_tracking.pdf}
...
\begin{figure}[H]
	\centering
		\includegraphics[width=0.95\textwidth]{dsdm_speed_tracking.pdf}
...
\begin{figure}[H]
	\centering
		\includegraphics[width=0.95\textwidth]{dsdm_nullspace.pdf}
...
\begin{figure}[H]
	\centering
		\includegraphics[width=0.95\textwidth]{dsdm_cstspd_shifts.pdf}
...
\begin{figure}[H]
	\centering
		\includegraphics[width=0.95\textwidth]{dsdm_cstspd_shifts_withprep.pdf}
...
\begin{figure}[H]
	\centering
		\includegraphics[width=0.95\textwidth]{dsdm_cstspd_shifts_withprep_zoom.pdf}
...
\begin{figure}[H]
	\centering
		\includegraphics[width=0.95\textwidth]{dsdm_compliant_HF.pdf}
...
\begin{figure}[H]
	\centering
		\includegraphics[width=0.95\textwidth]{dsdm_compliant_HS.pdf}
...
\begin{figure}[H]
	\centering
		\includegraphics[width=0.95\textwidth]{dsdm_ballon_down_shift_nice2.pdf}
	\caption{ Fast downshift during a contact with a compliant load }
...
\begin{figure}[H]
	\centering
		\includegraphics[width=0.95\textwidth]{dsdm_stiff_auto_new.pdf}
...
\begin{figure}[H]
	\centering
		\includegraphics[width=0.95\textwidth]{dsdm_stiff_auto_nullspace_wrong.pdf}
	\caption{ Fast downshift during a contact with a fixed stiff load }
