...
During a contact, impulsive forces can create a sudden change of velocity. Hence, if a DSDM actuator is used on a robotic system making contact with objects, its internal velocities can suddenly change. Since there is no impulsive forces on the motor side, the effect of contacts, from the actuator point-of-view, can be reduce to an impulsive torque applied to the output of the actuator. The output impulse $p_{ext}$ (integral of the impulsive force during the impact) is thus sufficient to characterized the velocity jumps:
%
\begin{align}
...
%
\begin{align}
\Delta w_o  &= w_o^+ - w_o^- = \frac{ p_{ext} }{ I_o + I_1 R_1^2}  \\
