...
Those results basically show that when the reflected inertia of M2 is much greater than that of M1, the impact affect only $w_1$ and $w_2$ remains unchanged through the impact. 

%
\begin{align}
\left[ \begin{array}{c} w_1^+ \\ \int{f_c dt} \end{array} \right] &= \Bigg[ \begin{array}{c c} -R_1 & 1 \\ -(I_o + R_1^2 I_1) & 0 \end{array} \Bigg] \left[ \begin{array}{c} w_0^- \\ w_1^- \end{array} \right] 
...
Generalized case, complex robot dynamic:
...
Proposed predictive controller: regulate as much as possible (up to velocity saturation limits) :
...
which will lead when an impact happen to 
%
\begin{align}
w_1^+ =  w_1^- + \Delta w_1 = -\hat{f}_{imp}( \vec{q} , \vec{\dot{q}}^- , J_c ) + f_{imp}( \vec{q} , \vec{\dot{q}}^- , J_c )  \approx 0
