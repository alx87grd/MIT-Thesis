...
\end{figure}
%
Fig. \ref{fig:dsdm_speed_tracking} shows the DSDM tracking speed set-points. Note that the noisy behavior during high-speed mode is only due to an implementation limitation. At low speed there is a quantization problem: there is a too small amounts of encoder ticks per control loop interval. 
%
\begin{figure}[H]
	\centering
...
\end{figure}
%
Note that the undesirable jump in velocity at Fig. \ref{fig:dsdm_cstspd_shifts} and Fig. \ref{fig:dsdm_cstspd_shifts_withprep}, actually happens at up-shifts (no impacts) and are caused by the transient response of the high-speed PID controller. This undesirable transient response was limited by implementation issue of the experimental set-up. Fig. \ref{fig:shifts} shows the same experiments when the DSDM controller is implemented on a FPGA at much higher sampling rate, which shows a much smoother output speed throughout the gear-shifting processes. 
%
\begin{figure}[htp]
	\centering
		\includegraphics[width=0.75\textwidth]{shift.pdf}
	\caption{Smoother shift behavior with a DSDM controller implemented at higher rate on a FPGA}
	\label{fig:shifts}
\end{figure}
